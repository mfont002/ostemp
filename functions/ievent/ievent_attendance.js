...
module.exports.getattendance = (event, context, callback) => {
  ...
  processDbUser = async (user) => {
    try {
      ...
      if (dest_id == 0) {
        let member_result = await data.transaction().query('insert into member' +
          '(username,' +
          'status,' +
          'creation_date,' +
          'activated,' +
          ...
          .query((r, x) => ['insert into member_group_map (member_id, group_id) values(:member_id, :group_id)', { member_id: r.insertId, group_id: x.insertId }])
          .query((r) => [`INSERT INTO member_meta (member_id,member_key,member_value) VALUES(:member_id,:member_key,:member_value)`],
            ...
            [{ member_id: r.insertId, member_key: 'last_name', member_value: user.last_name }],
              ...
              [{ member_id: r.insertId, member_key: 'activated', member_value: user.parent_last_name}],
              [{ member_id: r.insertId, member_key: 'last_name', member_value: user.parent_first_name}],
